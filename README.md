# MVP
- Someone must be able to become a User via username and password (invitation only for now)
...
- A User can only be active in one Room at a time
- A User must be able to join a room with other Users
- A User must be able to search for songs on youtube and add them to a library
- A User must be able to use Music box on any device (e.g mobile, desktop, ps3, etc.)
- A User must be able to add Songs from their Library to the current Rooms Queue
- A User must be able to chat with other Users
- A User must be able to enqueue songs into a Room
- A User must be able to change the order of their songs in the queue
...
```json
  "homepage": "https://myusername.github.io/musicbox-react",
```
...
To publish it at [https://myusername.github.io/musicbox-react](https://myusername.github.io/musicbox-react), run:
